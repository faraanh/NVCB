	...
	*/
	...
	function getTableList($skip_authentication = false) {
		...
		$arrTables = [
			...
			'DONVI' => ['DONVI', '', 'table.gif', 'HeThong'],
			'CanBo' => ['Canbo', '', 'table.gif', 'HeThong'],
			'CTV' => ['CTV', '', 'table.gif', 'CTVBM'],
			'LoaiCTV' => ['LoaiCTV', '', 'table.gif', 'CTVBM'],
			'BL3' => ['BL3', '', 'table.gif', 'CTVBM'],
			'PhanLoai_CTV' => ['Phanloai CTV', '', 'table.gif', 'CTVBM'],
			'BL2' => ['BL2', '', 'table.gif', 'CTVBM'],
			'XulyTin' => ['XulyTin', '', 'table.gif', 'CTVBM'],
			'TinBao' => ['TinBao', '', 'table.gif', 'CTVBM'],
			'Ketqua' => ['Ketqua', '', 'table.gif', 'CTVBM'],
			'STCN' => ['STCN', '', 'table.gif', 'SuuTra'],
			'STCNSTT' => ['STCNSTT', '', 'table.gif', 'SuuTra'],
			'DOITUONG' => ['DOITUONG', '', 'table.gif', 'SuuTra'],
		];
		...
		if($schema === null) {
			...
			$schema = [
				...
				'CanBo' => [
					...
					'IDMember' => [
						'appgini' => "VARCHAR(100) NOT NULL UNIQUE",
						'info' => [
							'caption' => 'IDMember',
							'description' => '',
						],
	...
	*/
	...
	function get_parent_tables($table) {
		...
		$parents = [
			...
			'BL3' => [
				'CTV' => ['IDCTV'],
				'PhanLoai_CTV' => ['Phanloai', 'CHPhanloai', 'CBPhanloai'],
			],
			'BL2' => [
				'BL3' => ['IDBL3'],
				'TinBao' => ['Tinbao'],
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			...
			'CanBo' => [
				'Donvi' => 'SELECT `DONVI`.`IDDonvi`, `DONVI`.`Tendonvi` FROM `DONVI` ORDER BY 2',
			],
			'CTV' => [
				'LoaiCTV' => 'SELECT `LoaiCTV`.`IDLoaiCTV`, `LoaiCTV`.`Ten` FROM `LoaiCTV` ORDER BY 2',
				'DVSD' => 'SELECT `DONVI`.`IDDonvi`, `DONVI`.`Tendonvi` FROM `DONVI` ORDER BY 2',
				'CBSD' => 'SELECT `CanBo`.`IDCanbo`, `CanBo`.`Hovaten` FROM `CanBo` LEFT JOIN `DONVI` as DONVI1 ON `DONVI1`.`IDDonvi`=`CanBo`.`Donvi` ORDER BY 2',
				'DVLap' => 'SELECT `DONVI`.`IDDonvi`, `DONVI`.`Tendonvi` FROM `DONVI` ORDER BY 2',
				'CBLap' => 'SELECT `CanBo`.`IDCanbo`, `CanBo`.`Hovaten` FROM `CanBo` LEFT JOIN `DONVI` as DONVI1 ON `DONVI1`.`IDDonvi`=`CanBo`.`Donvi` ORDER BY 2',
			],
			'LoaiCTV' => [
			],
			'BL3' => [
				'IDCTV' => 'SELECT `CTV`.`IDCTV`, `CTV`.`Bidanh` FROM `CTV` LEFT JOIN `LoaiCTV` as LoaiCTV1 ON `LoaiCTV1`.`IDLoaiCTV`=`CTV`.`LoaiCTV` LEFT JOIN `DONVI` as DONVI1 ON `DONVI1`.`IDDonvi`=`CTV`.`DVSD` LEFT JOIN `CanBo` as CanBo1 ON `CanBo1`.`IDCanbo`=`CTV`.`CBSD` LEFT JOIN `DONVI` as DONVI2 ON `DONVI2`.`IDDonvi`=`CTV`.`DVLap` LEFT JOIN `CanBo` as CanBo2 ON `CanBo2`.`IDCanbo`=`CTV`.`CBLap` ORDER BY 2',
				'CBPhanloai' => 'SELECT `PhanLoai_CTV`.`ID`, `PhanLoai_CTV`.`Phanloai` FROM `PhanLoai_CTV` ORDER BY 2',
				'CHPhanloai' => 'SELECT `PhanLoai_CTV`.`ID`, `PhanLoai_CTV`.`Phanloai` FROM `PhanLoai_CTV` ORDER BY 2',
				'Phanloai' => 'SELECT `PhanLoai_CTV`.`ID`, `PhanLoai_CTV`.`Phanloai` FROM `PhanLoai_CTV` ORDER BY 2',
			],
			'PhanLoai_CTV' => [
			],
			'BL2' => [
				'IDBL3' => 'SELECT `BL3`.`IDBL3`, IF(CHAR_LENGTH(if(`BL3`.`Ngaydauki`,date_format(`BL3`.`Ngaydauki`,\'%d/%m/%Y %H:%i\'),\'\')) || CHAR_LENGTH(if(`BL3`.`Ngaycuoiki`,date_format(`BL3`.`Ngaycuoiki`,\'%d/%m/%Y %H:%i\'),\'\')), CONCAT_WS(\'\', if(`BL3`.`Ngaydauki`,date_format(`BL3`.`Ngaydauki`,\'%d/%m/%Y %H:%i\'),\'\'), \'-\', if(`BL3`.`Ngaycuoiki`,date_format(`BL3`.`Ngaycuoiki`,\'%d/%m/%Y %H:%i\'),\'\')), \'\') FROM `BL3` LEFT JOIN `CTV` as CTV1 ON `CTV1`.`IDCTV`=`BL3`.`IDCTV` LEFT JOIN `PhanLoai_CTV` as PhanLoai_CTV1 ON `PhanLoai_CTV1`.`ID`=`BL3`.`CBPhanloai` LEFT JOIN `PhanLoai_CTV` as PhanLoai_CTV2 ON `PhanLoai_CTV2`.`ID`=`BL3`.`CHPhanloai` LEFT JOIN `PhanLoai_CTV` as PhanLoai_CTV3 ON `PhanLoai_CTV3`.`ID`=`BL3`.`Phanloai` ORDER BY 2',
				'Tinbao' => 'SELECT `TinBao`.`ID`, `TinBao`.`TenTinbao` FROM `TinBao` ORDER BY 2',
				'Xylytin' => 'SELECT `XulyTin`.`ID`, `XulyTin`.`Xulytin` FROM `XulyTin` ORDER BY 2',
				'Ketqua' => 'SELECT `Ketqua`.`ID`, `Ketqua`.`Ketqua` FROM `Ketqua` ORDER BY 2',
			],
			'XulyTin' => [
			],
			'TinBao' => [
			...
			'STCN' => [
				'DVLap' => 'SELECT `DONVI`.`IDDonvi`, `DONVI`.`Tendonvi` FROM `DONVI` ORDER BY 2',
				'DVQuanly' => 'SELECT `DONVI`.`IDDonvi`, `DONVI`.`Tendonvi` FROM `DONVI` ORDER BY 2',
				'Doituong' => 'SELECT `DOITUONG`.`IDDoituong`, `DOITUONG`.`Hovaten` FROM `DOITUONG` ORDER BY 2',
			],
			'STCNSTT' => [
				'IDSTCN' => 'SELECT `STCN`.`IDSTCN`, IF(    CHAR_LENGTH(`DOITUONG1`.`Hovaten`), CONCAT_WS(\'\',   `DOITUONG1`.`Hovaten`), \'\') FROM `STCN` LEFT JOIN `DONVI` as DONVI1 ON `DONVI1`.`IDDonvi`=`STCN`.`DVLap` LEFT JOIN `DONVI` as DONVI2 ON `DONVI2`.`IDDonvi`=`STCN`.`DVQuanly` LEFT JOIN `DOITUONG` as DOITUONG1 ON `DOITUONG1`.`IDDoituong`=`STCN`.`Doituong` ORDER BY 2',
			],
			'DOITUONG' => [
			],
		];
