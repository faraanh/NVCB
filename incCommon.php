<?php
	...
	*/
	...
	function get_table_groups($skip_authentication = false) {
		...
		$all_groups = ['CTVBM', 'SuuTra', 'HeThong'];
	...
	function get_sql_fields($table_name) {
		$sql_fields = [
			'DONVI' => "`DONVI`.`IDDonvi` as 'IDDonvi', `DONVI`.`Tendonvi` as 'Tendonvi', `DONVI`.`Hedonvi` as 'Hedonvi'",
			'CanBo' => "`CanBo`.`IDCanbo` as 'IDCanbo', `CanBo`.`IDMember` as 'IDMember', `CanBo`.`Hovaten` as 'Hovaten', `CanBo`.`Gioitinh` as 'Gioitinh', `CanBo`.`Chucvu` as 'Chucvu', IF(    CHAR_LENGTH(`DONVI1`.`Tendonvi`), CONCAT_WS('',   `DONVI1`.`Tendonvi`), '') as 'Donvi', `CanBo`.`Thongtin` as 'Thongtin'",
			'CTV' => "`CTV`.`IDCTV` as 'IDCTV', `CTV`.`Sodangky` as 'Sodangky', DATE_FORMAT(`CTV`.`Ngaydangky`, '%d/%m/%Y') as 'Ngaydangky', IF(    CHAR_LENGTH(`LoaiCTV1`.`Ten`), CONCAT_WS('',   `LoaiCTV1`.`Ten`), '') as 'LoaiCTV', `CTV`.`Bidanh` as 'Bidanh', DATE_FORMAT(`CTV`.`NgayPLgannhat`, '%d/%m/%Y') as 'NgayPLgannhat', DATE_FORMAT(`CTV`.`NgaydenhanPL`, '%d/%m/%Y') as 'NgaydenhanPL', IF(    CHAR_LENGTH(`DONVI1`.`Tendonvi`), CONCAT_WS('',   `DONVI1`.`Tendonvi`), '') as 'DVSD', IF(    CHAR_LENGTH(`CanBo1`.`Hovaten`), CONCAT_WS('',   `CanBo1`.`Hovaten`), '') as 'CBSD', IF(    CHAR_LENGTH(`DONVI2`.`Tendonvi`), CONCAT_WS('',   `DONVI2`.`Tendonvi`), '') as 'DVLap', IF(    CHAR_LENGTH(`CanBo2`.`Hovaten`), CONCAT_WS('',   `CanBo2`.`Hovaten`), '') as 'CBLap', `CTV`.`Status` as 'Status', `CTV`.`Diaban` as 'Diaban', `CTV`.`Doituong` as 'Doituong', `CTV`.`Vitri` as 'Vitri'",
			'LoaiCTV' => "`LoaiCTV`.`IDLoaiCTV` as 'IDLoaiCTV', `LoaiCTV`.`Loai` as 'Loai', `LoaiCTV`.`Ten` as 'Ten', `LoaiCTV`.`Ghichu` as 'Ghichu'",
			'BL3' => "`BL3`.`IDBL3` as 'IDBL3', IF(    CHAR_LENGTH(`CTV1`.`Bidanh`), CONCAT_WS('',   `CTV1`.`Bidanh`), '') as 'IDCTV', DATE_FORMAT(`BL3`.`Ngaydauki`, '%d/%m/%Y') as 'Ngaydauki', DATE_FORMAT(`BL3`.`Ngaycuoiki`, '%d/%m/%Y') as 'Ngaycuoiki', IF(    CHAR_LENGTH(`PhanLoai_CTV1`.`Phanloai`), CONCAT_WS('',   `PhanLoai_CTV1`.`Phanloai`), '') as 'CBPhanloai', IF(    CHAR_LENGTH(`PhanLoai_CTV2`.`Phanloai`), CONCAT_WS('',   `PhanLoai_CTV2`.`Phanloai`), '') as 'CHPhanloai', IF(    CHAR_LENGTH(`PhanLoai_CTV3`.`Phanloai`), CONCAT_WS('',   `PhanLoai_CTV3`.`Phanloai`), '') as 'Phanloai', `BL3`.`Nx_Dx` as 'Nx_Dx'",
			'PhanLoai_CTV' => "`PhanLoai_CTV`.`ID` as 'ID', `PhanLoai_CTV`.`Phanloai` as 'Phanloai'",
			'BL2' => "`BL2`.`IDBL2` as 'IDBL2', IF(    CHAR_LENGTH(if(`BL31`.`Ngaydauki`,date_format(`BL31`.`Ngaydauki`,'%d/%m/%Y %H:%i'),'')) || CHAR_LENGTH(if(`BL31`.`Ngaycuoiki`,date_format(`BL31`.`Ngaycuoiki`,'%d/%m/%Y %H:%i'),'')), CONCAT_WS('',   if(`BL31`.`Ngaydauki`,date_format(`BL31`.`Ngaydauki`,'%d/%m/%Y %H:%i'),''), '-', if(`BL31`.`Ngaycuoiki`,date_format(`BL31`.`Ngaycuoiki`,'%d/%m/%Y %H:%i'),'')), '') as 'IDBL3', DATE_FORMAT(`BL2`.`Ngaysinhhoat`, '%d/%m/%Y') as 'Ngaysinhhoat', DATE_FORMAT(`BL2`.`Ngaybaotin`, '%d/%m/%Y') as 'Ngaybaotin', IF(    CHAR_LENGTH(`TinBao1`.`TenTinbao`), CONCAT_WS('',   `TinBao1`.`TenTinbao`), '') as 'Tinbao', IF(    CHAR_LENGTH(`XulyTin1`.`Xulytin`), CONCAT_WS('',   `XulyTin1`.`Xulytin`), '') as 'Xylytin', IF(    CHAR_LENGTH(`Ketqua1`.`Ketqua`), CONCAT_WS('',   `Ketqua1`.`Ketqua`), '') as 'Ketqua'",
			'XulyTin' => "`XulyTin`.`ID` as 'ID', `XulyTin`.`Xulytin` as 'Xulytin'",
			'TinBao' => "`TinBao`.`ID` as 'ID', `TinBao`.`TenTinbao` as 'TenTinbao'",
			'Ketqua' => "`Ketqua`.`ID` as 'ID', `Ketqua`.`Ketqua` as 'Ketqua'",
			'STCN' => "`STCN`.`IDSTCN` as 'IDSTCN', `STCN`.`Sodangki` as 'Sodangki', DATE_FORMAT(`STCN`.`Ngaydangki`, '%d/%m/%Y') as 'Ngaydangki', DATE_FORMAT(`STCN`.`Ngaylap`, '%d/%m/%Y') as 'Ngaylap', `STCN`.`CBLap` as 'CBLap', IF(    CHAR_LENGTH(`DONVI1`.`Tendonvi`), CONCAT_WS('',   `DONVI1`.`Tendonvi`), '') as 'DVLap', `STCN`.`CBQuanly` as 'CBQuanly', IF(    CHAR_LENGTH(`DONVI2`.`Tendonvi`), CONCAT_WS('',   `DONVI2`.`Tendonvi`), '') as 'DVQuanly', IF(    CHAR_LENGTH(`DOITUONG1`.`Hovaten`), CONCAT_WS('',   `DOITUONG1`.`Hovaten`), '') as 'Doituong', `STCN`.`Status` as 'Status'",
			'STCNSTT' => "`STCNSTT`.`IDSTT` as 'IDSTT', `STCNSTT`.`Phanloai` as 'Phanloai', `STCNSTT`.`Danhmuc` as 'Danhmuc', `STCNSTT`.`Heloai` as 'Heloai', IF(    CHAR_LENGTH(`DOITUONG1`.`Hovaten`), CONCAT_WS('',   `DOITUONG1`.`Hovaten`), '') as 'IDSTCN'",
			'DOITUONG' => "`DOITUONG`.`IDDoituong` as 'IDDoituong', `DOITUONG`.`Hovaten` as 'Hovaten', DATE_FORMAT(`DOITUONG`.`Ngaysinh`, '%d/%m/%Y') as 'Ngaysinh', `DOITUONG`.`CCCD` as 'CCCD', `DOITUONG`.`TATS` as 'TATS'",
		];
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			'DONVI' => "`DONVI` ",
			'CanBo' => "`CanBo` LEFT JOIN `DONVI` as DONVI1 ON `DONVI1`.`IDDonvi`=`CanBo`.`Donvi` ",
			'CTV' => "`CTV` LEFT JOIN `LoaiCTV` as LoaiCTV1 ON `LoaiCTV1`.`IDLoaiCTV`=`CTV`.`LoaiCTV` LEFT JOIN `DONVI` as DONVI1 ON `DONVI1`.`IDDonvi`=`CTV`.`DVSD` LEFT JOIN `CanBo` as CanBo1 ON `CanBo1`.`IDCanbo`=`CTV`.`CBSD` LEFT JOIN `DONVI` as DONVI2 ON `DONVI2`.`IDDonvi`=`CTV`.`DVLap` LEFT JOIN `CanBo` as CanBo2 ON `CanBo2`.`IDCanbo`=`CTV`.`CBLap` ",
			'LoaiCTV' => "`LoaiCTV` ",
			'BL3' => "`BL3` LEFT JOIN `CTV` as CTV1 ON `CTV1`.`IDCTV`=`BL3`.`IDCTV` LEFT JOIN `PhanLoai_CTV` as PhanLoai_CTV1 ON `PhanLoai_CTV1`.`ID`=`BL3`.`CBPhanloai` LEFT JOIN `PhanLoai_CTV` as PhanLoai_CTV2 ON `PhanLoai_CTV2`.`ID`=`BL3`.`CHPhanloai` LEFT JOIN `PhanLoai_CTV` as PhanLoai_CTV3 ON `PhanLoai_CTV3`.`ID`=`BL3`.`Phanloai` ",
			'PhanLoai_CTV' => "`PhanLoai_CTV` ",
			'BL2' => "`BL2` LEFT JOIN `BL3` as BL31 ON `BL31`.`IDBL3`=`BL2`.`IDBL3` LEFT JOIN `TinBao` as TinBao1 ON `TinBao1`.`ID`=`BL2`.`Tinbao` LEFT JOIN `XulyTin` as XulyTin1 ON `XulyTin1`.`ID`=`BL2`.`Xylytin` LEFT JOIN `Ketqua` as Ketqua1 ON `Ketqua1`.`ID`=`BL2`.`Ketqua` ",
			'XulyTin' => "`XulyTin` ",
			'TinBao' => "`TinBao` ",
			'Ketqua' => "`Ketqua` ",
			'STCN' => "`STCN` LEFT JOIN `DONVI` as DONVI1 ON `DONVI1`.`IDDonvi`=`STCN`.`DVLap` LEFT JOIN `DONVI` as DONVI2 ON `DONVI2`.`IDDonvi`=`STCN`.`DVQuanly` LEFT JOIN `DOITUONG` as DOITUONG1 ON `DOITUONG1`.`IDDoituong`=`STCN`.`Doituong` ",
			'STCNSTT' => "`STCNSTT` LEFT JOIN `STCN` as STCN1 ON `STCN1`.`IDSTCN`=`STCNSTT`.`IDSTCN` LEFT JOIN `DOITUONG` as DOITUONG1 ON `DOITUONG1`.`IDDoituong`=`STCN1`.`Doituong` ",
			'DOITUONG' => "`DOITUONG` ",
		];
